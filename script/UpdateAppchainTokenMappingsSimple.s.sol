...
contract UpdateAppchainTokenMappingsSimple is Script {
    
    function run() public {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        address deployer = vm.addr(deployerPrivateKey);
        
        console.log("========== UPDATE APPCHAIN TOKEN MAPPINGS (SIMPLE) ==========");
        ...
        try cbm.getCrossChainConfig() returns (uint32 currentDestDomain, address currentDestBalanceManager) {
            ...
            console.log("Target destination domain:", rariDomain);
            console.log("Target destination BalanceManager:", rariBalanceManager);
