...
contract ExecuteLargeAppchainDeposits is Script {
    
    function run() public {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        address deployer = vm.addr(deployerPrivateKey);
        
        console.log("========== EXECUTE LARGE APPCHAIN DEPOSITS ==========");
        console.log("Large deposits to specific recipient");
        ...
        if (usdtBalance >= usdtAmount) {
            ...
            if (approveSuccess) {
                try cbm.deposit(sourceUSDT, usdtAmount, recipient) {
                    console.log("USDT deposit SUCCESS: 100,000 USDT");
        ...
        if (wbtcBalance >= wbtcAmount) {
            ...
            if (approveSuccess) {
                try cbm.deposit(sourceWBTC, wbtcAmount, recipient) {
                    console.log("WBTC deposit SUCCESS: 1 WBTC");
                } catch Error(string memory reason) {
        ...
        if (wethBalance >= wethAmount) {
            ...
            if (approveSuccess) {
                try cbm.deposit(sourceWETH, wethAmount, recipient) {
                    console.log("WETH deposit SUCCESS: 10 WETH");
                } catch Error(string memory reason) {
        ...
        console.log("Recipient:", recipient);
        console.log("Total deposited: 100,000 USDT + 1 WBTC + 10 WETH");
        console.log("These should arrive on Rari since Appchain works correctly");
        console.log("========== LARGE APPCHAIN DEPOSITS EXECUTED ==========");
    }
}