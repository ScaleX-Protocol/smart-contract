...
contract SetupChainBalanceManager is Script {
    ...
    function run() public {
        ...
        // Step 2: Whitelist USDT
        console.log("Step 2: Whitelisting USDT...");
        try cbm.addToken(APPCHAIN_USDT) {
            console.log("SUCCESS: USDT whitelisted successfully");
        } catch {
            console.log("ERROR: Failed to whitelist USDT - may already be whitelisted or need permissions");
        }
        
        // Step 3: Set token mapping
        console.log("Step 3: Setting token mapping USDT -> gsUSDT...");
        try cbm.setTokenMapping(APPCHAIN_USDT, RARI_GSUSDT) {
        ...
        try cbm.isTokenWhitelisted(APPCHAIN_USDT) returns (bool whitelisted) {
            console.log("USDT whitelisted:", whitelisted);
        } catch {
            console.log("Could not check USDT whitelist status");
        }
        
        try cbm.getTokenMapping(APPCHAIN_USDT) returns (address synthetic) {
            console.log("USDT -> Synthetic mapping:", synthetic);
            console.log("Expected gsUSDT:", RARI_GSUSDT);
            if (synthetic == RARI_GSUSDT) {
