...
/**
 * @title UpgradeBalanceManager
 * @dev Upgrade BalanceManager to latest implementation via Beacon pattern
 */
contract UpgradeBalanceManager is Script {
    // EIP1967 beacon storage slot
    bytes32 internal constant BEACON_SLOT = 0xa3f0ad74e5423aebfd80d3ef4346578335a9a72aeaee59ff6cb3582b35133d50;

    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        address balanceManagerProxy = vm.envAddress("BALANCE_MANAGER_PROXY");

        address deployer = vm.addr(deployerPrivateKey);

        console.log("=== BalanceManager Upgrade ===");
        console.log("Deployer:", deployer);
        console.log("BalanceManager Proxy:", balanceManagerProxy);

        // Get beacon address from proxy storage
        address beaconAddress = _getBeacon(balanceManagerProxy);
        ...
        console.log("\nDeploying new BalanceManager implementation...");
        ...
        if (verifiedImpl == address(newImpl)) {
            console.log("\n[SUCCESS] BalanceManager upgraded successfully!");
        } else {
