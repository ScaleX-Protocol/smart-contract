...
/**
 * @title MinimalAPPLEMarketOrder
 * @notice Place market BUY order to match against existing APPLE SELL limit order
 */
contract MinimalAPPLEMarketOrder is Script, DeployHelpers {
    ...
    function run() external {
        ...
        Currency apple = Currency.wrap(sxAPPLE);
        Currency usdc = Currency.wrap(sxUSDC);
        PoolKey memory poolKey = poolManager.createPoolKey(apple, usdc);
        IPoolManager.Pool memory pool = poolManager.getPool(poolKey);

        console.log("\n=== APPLE Market Order Test ===");
        console.log("APPLE Pool OrderBook:", address(pool.orderBook));
        ...
        // Price is $270, want to buy ~5 APPLE = need $1500 USDC
        uint256 usdcAmount = 1500e6; // $1500 USDC (6 decimals)
        ...
        console.log("\n[STEP 1] Deposited $1500 USDC to BalanceManager");
        ...
        console.log("\n[STEP 2] Placing market BUY order for $1500 USDC...");

        uint48 orderId;
        uint128 filled;
        try scalexRouter.placeMarketOrder(
            pool,
            1500e6, // $1500 USDC to spend (6 decimals)
            IOrderBook.Side.BUY,
            ...
            console.log("  Filled:", _filled / 1e18, "APPLE tokens");
        ...
        uint256 appleBalanceAfter = balanceManager.getBalance(marketOrderWallet, Currency.wrap(sxAPPLE));

        console.log("  USDC remaining:", usdcBalanceAfter / 1e6, "USDC");
        console.log("  APPLE gained:", appleBalanceAfter / 1e18, "APPLE");
        ...
        console.log("  Volume at best price:", bestBuy.volume / 1e18, "APPLE");
