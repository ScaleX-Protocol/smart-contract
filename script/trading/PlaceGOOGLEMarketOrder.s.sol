...
/**
 * @title MinimalGOOGLEMarketOrder
 * @notice Place market BUY order to match against existing GOOGLE SELL limit order
 */
contract MinimalGOOGLEMarketOrder is Script, DeployHelpers {
    ...
    function run() external {
        ...
        Currency google = Currency.wrap(sxGOOGLE);
        Currency usdc = Currency.wrap(sxUSDC);
        PoolKey memory poolKey = poolManager.createPoolKey(google, usdc);
        IPoolManager.Pool memory pool = poolManager.getPool(poolKey);

        console.log("\n=== GOOGLE Market Order Test ===");
        console.log("GOOGLE Pool OrderBook:", address(pool.orderBook));
        ...
        // Price is $180, want to buy ~5 GOOGLE = need $900 USDC
        ...
        console.log("\n[STEP 2] Placing market BUY order for $1,000 USDC...");

        uint48 orderId;
        uint128 filled;
        try scalexRouter.placeMarketOrder(
            pool,
            1000e6, // $1,000 USDC to spend (6 decimals)
            IOrderBook.Side.BUY,
            ...
            console.log("  Filled:", _filled / 1e18, "GOOGLE tokens");
        ...
        uint256 googleBalanceAfter = balanceManager.getBalance(marketOrderWallet, Currency.wrap(sxGOOGLE));

        console.log("  USDC remaining:", usdcBalanceAfter / 1e6, "USDC");
        console.log("  GOOGLE gained:", googleBalanceAfter / 1e18, "GOOGLE");
        ...
        console.log("  Volume at best price:", bestBuy.volume / 1e18, "GOOGLE");
