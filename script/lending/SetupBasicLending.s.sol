...
contract SetupBasicLending is Script, DeployHelpers {
    ...
    function _displayLendingStatus() internal {
        ...
        try lendingManager.getUserSupply(deployer, usdc) returns (uint256 supply) {
            console.log("USDC Supply:", supply / 10**6);
        } catch {
            console.log("USDC Supply: Not available");
        }
        
        try lendingManager.getUserDebt(deployer, usdc) returns (uint256 debt) {
            console.log("USDC Debt:", debt / 10**6);
        } catch {
            console.log("USDC Debt: Not available");
        }
        
        try lendingManager.calculateInterestRate(usdc) returns (uint256 rate) {
            console.log("USDC Interest Rate:", rate / 100, "%");
        } catch {
            console.log("USDC Interest Rate: Not configured");
        }
