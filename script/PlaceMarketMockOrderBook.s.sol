...
contract PlaceMarketMockOrderBook is Script, DeployHelpers {
    ...
    function runConfigurable(uint8 numBuyOrders, uint8 numSellOrders, uint256 ethAmount, uint256 usdcAmount) public {
        uint256 deployerPrivateKey = getDeployerKey2();
        vm.startBroadcast(deployerPrivateKey);

        deployerAddress = vm.addr(deployerPrivateKey);

        placeMarketOrdersETHUSDCConfigurable(numBuyOrders, numSellOrders, ethAmount, usdcAmount);
        verifyMarketOrders();
    ...
    function placeMarketOrdersETHUSDCConfigurable(
        uint8 numBuyOrders,
        uint8 numSellOrders,
        uint256 ethAmount,
        uint256 usdcAmount
    ) private {
        console.log("\n=== Placing Market Orders on ETH/USDC (Configurable) ===");
        ...
        // Place configurable number of market BUY orders
        if (numBuyOrders > 0) {
            _placeMarketBuyOrders(pool, numBuyOrders);
        }

        // Place configurable number of market SELL orders
        if (numSellOrders > 0) {
            _placeMarketSellOrders(pool, numSellOrders);
        }

        // Print summary
        console.log("\nMarket orders placed:");
        console.log("- %s market BUY orders (buying ETH with USDC)", numBuyOrders);
        console.log("- %s market SELL orders (selling ETH for USDC)", numSellOrders);
    }
    ...
    function _placeMarketBuyOrders(IPoolManager.Pool memory pool, uint8 numOrders) private {
        ...
        for (uint8 i = 0; i < numOrders; i++) {
            ...
            (uint48 orderId, uint128 filled) =
                gtxRouter.placeMarketOrder(pool, quantity, IOrderBook.Side.BUY, depositAmount, minOutAmount);
    ...
    function _placeMarketSellOrders(IPoolManager.Pool memory pool, uint8 numOrders) private {
        ...
        for (uint8 i = 0; i < numOrders; i++) {
            ...
            (uint48 orderId, uint128 filled) =
                gtxRouter.placeMarketOrder(pool, quantity, IOrderBook.Side.SELL, depositAmount, minOutAmount);
    ...
    function verifyMarketOrders() private {
        ...
        for (uint256 i = 0; i < marketBuyOrderIds.length; i++) {
            _checkOrderDetails(
                weth, usdc, marketBuyOrderIds[i], string(abi.encodePacked("Market BUY #", uint2str(i + 1)))
            );
        }
        ...
        for (uint256 i = 0; i < marketSellOrderIds.length; i++) {
            _checkOrderDetails(
                weth, usdc, marketSellOrderIds[i], string(abi.encodePacked("Market SELL #", uint2str(i + 1)))
            );
        }
