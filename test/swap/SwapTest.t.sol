...
contract SwapTest is Test {
    ...
    function testWethToUsdcSwap() public {
        ...
        router.placeLimitOrder(
            ...
            0 // no additional deposit, already deposited
        );
        vm.stopPrank();

        // Bob swaps USDC for WETH (with auto-deposit from wallet)
        vm.startPrank(bob);
        usdc.approve(address(balanceManager), type(uint256).max);
    ...
    function testUsdcToWethSwap() public {
        ...
        // Bob swaps WETH for USDC (with auto-deposit from wallet)
        vm.startPrank(bob);
        weth.approve(address(balanceManager), type(uint256).max);
    ...
    function testSwapWithExistingBalance() public {
        // Alice deposits WETH and places a sell order
        vm.startPrank(alice);
        balanceManager.deposit(Currency.wrap(address(weth)), 10 ether, alice, alice);
        ...
        // Bob first deposits USDC to BalanceManager
        vm.startPrank(bob);
        balanceManager.deposit(Currency.wrap(address(usdc)), 5000 * 1e6, bob, bob);

        uint256 balanceBefore = balanceManager.getBalance(bob, Currency.wrap(address(usdc)));
        console.log("Bob USDC balance in BalanceManager before swap:", balanceBefore);

        // Bob swaps using existing balance (depositAmount = 0)
        uint256 receivedWeth = router.swap(
            Currency.wrap(address(usdc)),
            Currency.wrap(address(weth)),
            2000 * 1e6, // 2000 USDC
            ...
            2,
            bob,
            0 // depositAmount = 0 means use existing balance
        );
        vm.stopPrank();
    ...
    function testSwapWithPartialDeposit() public {
        // Alice deposits WETH and places a sell order
        vm.startPrank(alice);
        balanceManager.deposit(Currency.wrap(address(weth)), 10 ether, alice, alice);
        ...
        // Bob first deposits some USDC to BalanceManager
        vm.startPrank(bob);
        balanceManager.deposit(Currency.wrap(address(usdc)), 1000 * 1e6, bob, bob);
        usdc.approve(address(balanceManager), type(uint256).max);

        uint256 balanceBefore = balanceManager.getBalance(bob, Currency.wrap(address(usdc)));
        console.log("Bob USDC balance in BalanceManager before swap:", balanceBefore);

        // Bob swaps 2000 USDC total, depositing only 1000 more (has 1000 already)
        uint256 receivedWeth = router.swap(
            Currency.wrap(address(usdc)),
            Currency.wrap(address(weth)),
            2000 * 1e6, // 2000 USDC total to swap
            0,
            2,
            bob,
            1000 * 1e6 // deposit only 1000 USDC more from wallet
        );
        vm.stopPrank();
