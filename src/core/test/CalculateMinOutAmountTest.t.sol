...
contract CalculateMinOutAmountTest is Test {
    ...
    function testCalculateMinOutAmountBasic() public {
        ...
        uint128 minOutAmount = gtxRouter.calculateMinOutAmountForMarket(pool, quoteAmount, IOrderBook.Side.BUY, slippageBps);
        ...
        uint256 expectedMinOut = (expectedAfterFees * (10000 - slippageBps)) / 10000;
    ...
    function testPlaceMarketOrderWithCalculatedMinOut() public {
        ...
        uint128 minOutAmount = gtxRouter.calculateMinOutAmountForMarket(pool, depositAmount, IOrderBook.Side.BUY, slippageBps);
        ...
        assertApproxEqRel(filled, minOutAmount * 10500 / 10000, 5e16, "Filled should be close to calculated minimum * (1 + slippage)");
    }
    ...
    function testCalculateMinOutAmountHighSlippage() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IOrderBookErrors.InvalidSlippageTolerance.selector, 11000));
        gtxRouter.calculateMinOutAmountForMarket(pool, 1000e6, IOrderBook.Side.BUY, 11000); // 110%
    }
    ...
    function testCalculateMinOutAmountForMarketSell() public {
        ...
        vm.startPrank(buyer);
        mockUSDC.mint(buyer, 10000e6); // 10,000 USDC
        ...
        uint128 minQuoteFromSell = gtxRouter.calculateMinOutAmountForMarket(pool, baseAmount, IOrderBook.Side.SELL, slippageBps);
    ...
    function testCalculateMinOutAmountForMarketBoth() public {
        ...
        uint128 minBaseFromMarket = gtxRouter.calculateMinOutAmountForMarket(pool, quoteAmount, IOrderBook.Side.BUY, 200);
        ...
        uint128 minQuoteFromMarket2 = gtxRouter.calculateMinOutAmountForMarket(pool, baseAmount, IOrderBook.Side.SELL, 200);
    ...
    function testCalculateMinOutAmountForMarketEdgeCases() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IOrderBookErrors.InvalidSlippageTolerance.selector, 10001));
        gtxRouter.calculateMinOutAmountForMarket(pool, 1000e6, IOrderBook.Side.BUY, 10001);
