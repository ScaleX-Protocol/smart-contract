...
function calculateTotalLiquidity(levels) {
  let total = BigInt(0);
...
function calculateDepthLiquidity(levels, midPrice, percentDepth, isBid) {
  let totalLiquidity = BigInt(0);
  const threshold = isBid ? midPrice * (1 - percentDepth / 100) : midPrice * (1 + percentDepth / 100);
...
function calculateLiquidityMetrics(orderBook, symbol2) {
  ...
  const spreadBps = spread / midPrice * 1e4;
...
var fetchOrderBook = (nodeRuntime) => {
...
var onCronTrigger = (runtime2) => {
  runtime2.log("\uD83D\uDD04 ScaleX Liquidity Calculator Starting...");
  const orderBook = runtime2.runInNodeMode(fetchOrderBook, simpleConsensus())().result();
  if (!orderBook || !orderBook.bids || !orderBook.asks) {
    throw new Error("Invalid order book data received");
  }
  runtime2.log(`✅ Order book loaded: ${orderBook.bids.length} bids, ${orderBook.asks.length} asks`);
  runtime2.log(`\uD83D\uDCCA Last Update ID: ${orderBook.lastUpdateId}`);
  runtime2.log("\uD83D\uDCA7 Calculating liquidity metrics...");
  const metrics = calculateLiquidityMetrics(orderBook, runtime2.config.symbol);
  runtime2.log("============================================================");
  runtime2.log(`\uD83D\uDCC8 LIQUIDITY METRICS FOR ${metrics.symbol}`);
  runtime2.log("============================================================");
  runtime2.log(`\uD83D\uDD50 Timestamp: ${new Date(metrics.timestamp).toISOString()}`);
  runtime2.log(`\uD83D\uDCB1 Mid Price: ${metrics.midPrice}`);
  runtime2.log(`\uD83D\uDCCA Spread: ${toFixed(metrics.spread, 2)} (${toFixed(metrics.spreadBps, 2)} bps)`);
  runtime2.log(`\uD83D\uDCCA Order Book Depth: ${metrics.bidLevels} bids, ${metrics.askLevels} asks`);
  runtime2.log(`\uD83D\uDCA7 Total Liquidity:`);
  runtime2.log(`   Bids: ${toFixed(metrics.totalBidLiquidityETH, 6)} ETH`);
  runtime2.log(`   Asks: ${toFixed(metrics.totalAskLiquidityETH, 6)} ETH`);
  runtime2.log(`   Combined: ${toFixed(metrics.totalBidLiquidityETH + metrics.totalAskLiquidityETH, 6)} ETH`);
  runtime2.log(`\uD83D\uDCCF Liquidity Depth:`);
  runtime2.log(`   1% - Bids: ${toFixed(metrics.liquidity1PercentDepth.bids, 6)} ETH, Asks: ${toFixed(metrics.liquidity1PercentDepth.asks, 6)} ETH`);
  runtime2.log(`   5% - Bids: ${toFixed(metrics.liquidity5PercentDepth.bids, 6)} ETH, Asks: ${toFixed(metrics.liquidity5PercentDepth.asks, 6)} ETH`);
  runtime2.log("============================================================");
  return metrics;
};
